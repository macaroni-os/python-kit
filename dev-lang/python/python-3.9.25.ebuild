...
SRC_URI="https://www.python.org/ftp/python/3.9.25/Python-3.9.25.tar.xz -> Python-3.9.25.tar.xz"
LICENSE="PSF-2"
SLOT="3.9"
KEYWORDS="*"
PATCHES=(
	"${FILESDIR}/3.9/0001-Install-libpythonX.Y.a-in-usr-lib-instead-of-usr-lib.patch"
	"${FILESDIR}/3.9/0005-Improve-distutils-C-support.patch"
	"${FILESDIR}/3.9/0008-Install-shared-libs-in-LIBDEST.patch"
	"${FILESDIR}/3.9/0014-bpo-45433-Do-not-link-libpython-against-libcrypt.patch"
)
...
src_install() {
	local libdir=${ED}/usr/lib/python3.9
	emake DESTDIR="${D}" altinstall
	# Fix collisions between different slots of Python.
	rm "${ED}/usr/$(get_libdir)/libpython3.so" || die
	# Cheap hack to get version with ABIFLAGS
	local abiver=$(cd "${ED}/usr/include"; echo python*)
	if [[ ${abiver} != python3.9 ]]; then
	  # Replace python3.X with a symlink to python3.Xm
	  rm "${ED}/usr/bin/python3.9" || die
	  dosym "${abiver}" "/usr/bin/python3.9"
	  # Create python3.X-config symlink
	  dosym "${abiver}-config" "/usr/bin/python3.9-config"
	  # Create python-3.5m.pc symlink
	  dosym "python-3.9.pc" "/usr/$(get_libdir)/pkgconfig/${abiver/3.9/-3.9}.pc"
	fi
	if has_version dev-libs/libffi[pax_kernel]; then
	...
	use tk || rm -r "${ED}/usr/bin/idle3.9" "${libdir}/"{idlelib,tkinter,test/test_tk*} || die
	...
	newconfd "${FILESDIR}/pydoc.conf" pydoc-3.9
	newinitd "${FILESDIR}/pydoc.init" pydoc-3.9
	sed \
	  -e "s:@PYDOC_PORT_VARIABLE@:PYDOC3_9_PORT:" \
	  -e "s:@PYDOC@:pydoc3.9:" \
	  -i "${ED}/etc/conf.d/pydoc-3.9" \
	  "${ED}/etc/init.d/pydoc-3.9" || die "sed failed"
	local -x EPYTHON=python3.9
	...
	doman "${ED}"/share/man/man1/python3.9.1
	# remove uncompressed man files
	rm -r "${ED}"/share/man
}
pkg_preinst() {
	if has_version "<${CATEGORY}/${PN}-3.9" ; then
